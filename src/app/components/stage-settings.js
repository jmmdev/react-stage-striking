...
export default function StageSettings({stages, active, setActive, setShowSettings, language}) {
    ...
    return (
        <div className="fixed flex justify-center items-center w-screen h-screen top-0 left-0 bg-black/80 py-16">
            <div className="max-w-full w-fit max-h-96 lg:max-h-full bg-slate-600 rounded-xl overflow-y-auto">
                ...
                <div className="flex flex-col px-4">
                    {
                        stages.map((stage, index) => {
                            ...
                            return (
                                <div className={`flex flex-col sm:flex-row justify-between items-center p-4 gap-2 sm:gap-8 xl:gap-16 ${index < stages.length - 1 ? "border-b border-slate-400/50" : ""}`} key={stage.index}>
                                    <p className="truncate text-xl text-zinc-100 font-medium">{stage.name[language]}</p>
